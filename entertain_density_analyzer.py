...
从词汇角度分析男性和女性讨论内容中娱乐相关词汇的出现情况

功能：
1. 使用预定义的娱乐名词词表（entertainment_nouns）
2. 分析这些词在男性和女性发表内容中的出现频次，计算"entertain density"
3. 比较不同性别的平均entertain density和分布差异（Post级别、User级别、省份级别）
"""
...
def load_entertain_vocabulary(year):
    ...
    print(f"✓ 已加载娱乐词表，包含 {len(vocab)} 个词")
    return vocab
...
def calculate_entertain_density_fast(content, entertain_vocab):
    """快速计算entertain density（基于字符长度，不需要分词）

    直接检查entertain_vocab中的词在清理后的文本中的出现，
    计算占有的字符长度除以语句总长度。
    """
    ...
    # 计算娱乐词汇在文本中占有的字符长度
    entertain_char_length = 0
    for word in entertain_vocab:
        # 计算该词在文本中出现的所有位置的总字符长度
        count = cleaned_text.count(word)
        if count > 0:
            entertain_char_length += len(word) * count

    # entertain density = 娱乐词汇字符长度 / 总字符长度
    density = entertain_char_length / total_length if total_length > 0 else 0.0
    return density


def calculate_post_density(year, entertain_vocab, force_recalculate=False):
    """计算post级别的entertain density（核心函数，只计算一次）
    ...
    - gender: 性别
    - province: 省份（已转换为名称）
    - density: entertain density值

    Args:
        year: 年份
        entertain_vocab: 娱乐词表
        force_recalculate: 是否强制重新计算

    Returns:
        pd.DataFrame: post级别的density数据
    """
    post_density_file = os.path.join(
        OUTPUT_DIR, f"entertain_post_density_{year}.parquet"
    )
    ...
    # 重新计算
    print(f"\n开始计算 {year} 年post级别的entertain density...")
    ...
    for file_path in tqdm(parquet_files, desc="计算entertain density"):
        try:
            # 读取必要的列
            required_columns = [
                "weibo_id",
            ...
            df["density"] = df["weibo_content"].apply(
                lambda x: calculate_entertain_density_fast(x, entertain_vocab)
            )
...
def calculate_province_stats(
    ...
    province_post_file = os.path.join(
        OUTPUT_DIR, f"entertain_province_post_density_{year}.parquet"
    )
    province_user_file = os.path.join(
        OUTPUT_DIR, f"entertain_province_user_density_{year}.parquet"
    )
...
def calculate_district_stats(
    ...
    district_post_file = os.path.join(
        OUTPUT_DIR, f"entertain_district_post_density_{year}.parquet"
    )
    district_user_file = os.path.join(
        OUTPUT_DIR, f"entertain_district_user_density_{year}.parquet"
    )
...
def print_stats(stats, year, level="post"):
    ...
    print(f"\n{'='*60}")
    print(f"{level_label} Entertain Density统计结果")
    print(f"{'='*60}")
    ...
    if stats:
        ...
        stats_file = os.path.join(
            OUTPUT_DIR,
            f"{'entertain_density' if level == 'post' else 'entertain_user_density'}_stats_{year}.parquet",
        )
...
def visualize_density_distribution(data, year, level="post", n_plot_points=1000):
    """可视化entertain density的分布差异（post和user级别共用）
    ...
    level_label = "Post级别" if level == "post" else "User级别"
    density_label = (
        "Entertain Density" if level == "post" else "用户平均Entertain Density"
    )

    print(f"\n开始绘制{level_label} entertain density分布对比图...")
    ...
    for gender, densities in gender_densities.items():
        ...
        for density in non_zero_densities:
            plot_data.append({"gender": gender, "entertain_density": density})
    ...
    df_plot["EntertainDensity"] = df_plot["entertain_density"]
    ...
    ax3 = axes[1, 0]
    stats = (
        df_plot.groupby("Gender")["EntertainDensity"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    ...
    if len(gender_labels) == 2:
        ...
        if len(group1_data) > 0 and len(group2_data) > 0:
            ...
            ax4.text(
                ...
                f"------------------\n"
                f"结果: {p_text}\n"
                f"结论: {'男性entertain density显著更高' if p_val < 0.05 else '分布无显著差异'}",
    ...
    fig_filename = f"{'entertain_density' if level == 'post' else 'entertain_user_density'}_distribution_{year}.pdf"
...
def visualize_province_density_comparison(year):
    """绘制省份级别的Entertain Density对比图（4个子图）
    ...
    """
    print(f"\n开始绘制 {year} 年省份级别Entertain Density对比图...")
    ...
    fig.suptitle(
        f"{year}年各省份Entertain Density性别差异分析", fontsize=18, fontweight="bold"
    )
    ...
    fig_path = os.path.join(
        OUTPUT_DIR, f"entertain_province_density_comparison_{year}.pdf"
    )
    plt.savefig(fig_path, format="pdf", bbox_inches="tight", dpi=300)
    plt.close()
    print(f"图表已保存到: {fig_path}")
    print(f"\n省份级别Entertain Density对比图绘制完成\n")


def visualize_district_density_comparison(year):
    """绘制地区级别的Entertain Density对比图（4个子图）
    ...
    4. User级别平均density（>0，带99% CI）
    """
    print(f"\n开始绘制 {year} 年地区级别Entertain Density对比图...")
    ...
    fig.suptitle(
        f"{year}年各地区Entertain Density性别差异分析", fontsize=18, fontweight="bold"
    )
    ...
    # 保存图表
    fig_path = os.path.join(
        OUTPUT_DIR, f"entertain_district_density_comparison_{year}.pdf"
    )
    plt.savefig(fig_path, format="pdf", bbox_inches="tight", dpi=300)
    plt.close()
    print(f"图表已保存到: {fig_path}")
    print(f"\n地区级别Entertain Density对比图绘制完成\n")
...
def analyze_entertain_density(year, force_recalculate=False, force_reanalyze=False):
    """Entertain Density分析主入口函数
    ...
    """
    print(f"\n{'='*60}")
    print(f"开始分析 {year} 年Entertain Density（所有级别）")
    print(f"{'='*60}")

    # 加载娱乐词表
    entertain_vocab = load_entertain_vocabulary(year)
    ...
    print(f"{'='*60}")
    post_density_df = calculate_post_density(year, entertain_vocab, force_recalculate)
    if post_density_df is None or len(post_density_df) == 0:
    ...
    print(f"\n{'='*60}")
    print(f"{year} 年Entertain Density分析完成（所有级别）")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    fire.Fire(
        {
            "analyze": analyze_entertain_density,
        }
    )
