...
def add_demographic_for_all():
    ...
    for current_date in date_range:
        ...
        file_path = os.path.join(
            f"user_count/count-{current_date.year}",
        ...
        output_file_path = os.path.join(
            output_folder, f"{current_date.strftime('%Y-%m-%d')}.parquet"
        )

        if os.path.exists(file_path):
            daily_df = pd.read_parquet(file_path)
            daily_df[["gender", "age", "province"]] = daily_df.apply(
                demographic_processor.add_demographic, axis=1, result_type="expand"
            )
            daily_df.to_parquet(output_file_path, engine="fastparquet", index=False)

        log(
            f"Processing date: {current_date} finished, time: {int(time.time()) - date_start_time}"
        )
    ...
    for year in yearly:
        ...
        if os.path.exists(file_path):
            ...
            year_df[["gender", "age", "province"]] = year_df.apply(
                demographic_processor.add_demographic, axis=1, result_type="expand"
            )
    ...
    if year_dfs:
        ...
        yearly_df.to_parquet(
            "user_count2/yearly_user_counts.parquet", engine="fastparquet", index=False
        )


def gender_analysis():
    yearly_df = pd.read_parquet(
        "user_count2/yearly_user_counts.parquet", engine="fastparquet"
    )
    ...
    for year in year_range:
        ...
        female_ratio = (
            year_data_year[year_data_year["gender"] == "女"].shape[0]
            / year_data_year.shape[0]
        )
        male_ratio = 1 - female_ratio
        year_data.append({"year": year, "male": male_ratio, "female": female_ratio})
    ...
    # 绝对数量变化的面积图
    axes[2].plot(absolute_counts["year"], absolute_counts["female"], label="Female")
    axes[2].plot(absolute_counts["year"], absolute_counts["male"], label="Male")
    axes[2].plot(absolute_counts["year"], absolute_counts["missing"], label="Missing")
...
def age_analysis():
    ...
    for year in year_range:
        year_data_year = yearly_df[yearly_df["year"] == year]
        age_groups = pd.cut(
            year_data_year["age"], bins=bins, labels=labels, right=False
        )
...
def female_ratio_by_region():
    ...
    for year in year_range:
        ...
        for region in region_to_province:
            region_data = year_data_year[
                year_data_year["province"].map(province_to_region) == region
            ]
            female_ratio = (
                region_data[region_data["gender"] == "女"].shape[0]
                / region_data.shape[0]
...
def average_age_by_region():
    ...
    for year in year_range:
        ...
        for region in region_to_province:
            ...
            avg_age = (
                region_data["age"].mean()
                if not region_data["age"].isnull().all()
                else 0
            )
...
def average_age_by_gender():
    ...
    for year in year_range:
        ...
        overall_avg_age.append(year_data_year["age"].mean())
        male_avg_age.append(
            year_data_year[year_data_year["gender"] == "男"]["age"].mean()
        )
        female_avg_age.append(
            year_data_year[year_data_year["gender"] == "女"]["age"].mean()
        )
...
def gender_activity_analysis():
    ...
    for year in year_range:
        ...
        female_activity = year_data_year[year_data_year["gender"] == "女"][
            "count"
        ].sum()
        ...
        ratio_counts.append(
            {
                ...
                "missing": (
                    missing_activity / total_activity if total_activity > 0 else 0
                ),
            }
        )
    ...
    axes[0].plot(absolute_df["year"], absolute_df["female"], label="Female")
    axes[0].plot(absolute_df["year"], absolute_df["male"], label="Male")
    axes[0].plot(absolute_df["year"], absolute_df["missing"], label="Missing")
...
def region_activity_analysis():
    ...
    for year in year_range:
        ...
        for region in region_to_province:
            ...
            year_ratio[region] = (
                region_activity / total_activity if total_activity > 0 else 0
            )

        # 给ratio加上一个missing
        missing_activity_ratio = 1 - sum(
            year_ratio[region] for region in region_to_province
        )
    ...
    axes[1].stackplot(
        ...
        ratio_data["West"],
        ratio_data["Central"],
        ratio_data["Northeast"],
        ratio_data["Missing"],
        labels=["East", "West", "Central", "Northeast", "Missing"],
    )
...
def age_activity_analysis():
    ...
    for year in year_range:
        ...
        for label in labels:
            ...
            year_ratio[label] = (
                age_group_activity / total_activity if total_activity > 0 else 0
            )
    ...
    axes[1].stackplot(
        ...
        ratio_data["12-16"],
        ratio_data["Missing"],
        labels=["0-4", "4-8", "8-12", "12-16", "Missing"],
    )
